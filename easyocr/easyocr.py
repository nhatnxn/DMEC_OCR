...
from paddle_detector import text_detector
...
from paddle_detector import PaddleDetection
...
class Reader(object):
    ...
    def detect(self, img, min_size = 20, text_threshold = 0.7, low_text = 0.4,\
        ...
        # print('horizon_list: ',horizontal_list)
        ...
        if ((batch_size == 1) or (self.device == 'cpu')) and not rotation_info:
            ...
            for bbox in horizontal_list:
                ...
                result0 = get_text(self.character, imgH, int(max_width), self.recognizer, self.converter, image_list,\
                              ...
                              workers, self.device)
                result += result0
            print('result_horizontal: ', result)
    ...
    def readtext(self, image, decoder = 'greedy', beamWidth= 5, batch_size = 1,\
        ...
        horizontal_list = []
        # paddle = PaddleOCR(lang='latin')
        # free_list = paddle.ocr(img, rec=False, cls=True)

        pp_detect = PaddleDetection()
        free_list, cls_res = pp_detect.detect_box(img)

        # print('horizontal_list: ', horizontal_list)
        # print('free_list: ', free_list)
        x_mean, y_mean = [], []
        for dis in free_list:
            x_mean.append(self.distance(dis[0], dis[1]))
            y_mean.append(self.distance(dis[1], dis[2]))
        if sum(x_mean) < sum(y_mean):
            # cls_res=1

            img = cv2.rotate(img, cv2.cv2.ROTATE_90_CLOCKWISE)
            free_list, cls_res = pp_detect.detect_box(img)

        free_list = free_list.tolist()
        # print(type(free_list))
        # print(len(free_list))
        free_list.sort(key=lambda x:x[0][0])
        data = free_list

        # create a list of the gaps between the consecutive values
        gaps = [y[0][0] - x[0][0] for x, y in zip(data[:-1], data[1:])]
        ...
        #     x0 = str(int(block[0][0] + 0.99999)).rjust(4, "0")
        #     y0 = str(int(block[0][1] + 0.99999)).rjust(4, "0")
            
        #     sortkey = y0 + x0
        #     print(sortkey)
        #     blocks.append([block, sortkey])
        
        # blocks.sort(key = lambda x: x[0][1], reverse=False)
        # blocks = [i[0] for i in blocks]

        # print(len(free_list))
        result = self.recognize(img_cv_grey, horizontal_list, blocks,\
        ...
        for i,res in enumerate(result):
            ...
            cv2.line(img, (int(box[0][0]), int(box[0][1])), (int(box[1][0]),int(box[1][1])), color, 1)
            cv2.line(img, (int(box[1][0]), int(box[1][1])), (int(box[2][0]),int(box[2][1])), color, 1)
            cv2.line(img, (int(box[2][0]), int(box[2][1])), (int(box[3][0]),int(box[3][1])), color, 1)
            cv2.line(img, (int(box[3][0]), int(box[3][1])), (int(box[0][0]),int(box[0][1])), color, 1)
            # image = cv2.rectangle(img, (int(box[0][0]), int(box[0][1])), (int(box[2][0]),int(box[2][1])), color, 1)
            cv2.putText(img, str(i+1), (int(box[0][0])-10, int(box[0][1])), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (36,12,225), 1)
            cv2.imwrite('img_91.jpg', img)
...
if __name__ == '__main__':
    img_path = '/content/test_9.png'

    ocr = Reader(['en','vi'], detector=True, gpu=True)
